...
import { ASPSP, EndUserAgreement, Requisition, BalanceData, TransactionData, AccountDetailData } from './types';
...
export default class Nordigen {
   ...
   */
  constructor(accessToken : string, endpoint : string = "https://ob.nordigen.com/api") {
   ...
   */
  async makeRequest(path : string, method : string = "GET", body : object = {}) {
   ...
   */
  async getASPSPsForCountry(countryCode : string) : Promise<ASPSP[]> {
   ...
   */
  async createEndUserAgreement(enduser_id : string, aspsp_id : string, max_historical_days : number = 90) : Promise<EndUserAgreement> {
   ...
   */
  async createRequisition(enduser_id : string, redirect : string, reference : string, agreements : EndUserAgreement[] = [], user_language : string | undefined = undefined) : Promise<Requisition> {
  ...
  async getRequisitionLink(aspsp_id : string) : Promise<string | false> {
    const response = await this.makeRequest(`/requisitions/${aspsp_id}/links/`, 'POST');
